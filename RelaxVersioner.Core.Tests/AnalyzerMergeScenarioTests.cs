...
public sealed class AnalyzerMergeScenarioTests
{
    ...
    public async Task LookupVersionLabel_SimpleMerge_ChoosesLargerVersion()
    {
        ...
        try
        {
            ...
            await TestUtilities.RunGitCommandAsync(tempPath, "checkout -b feature");
            File.WriteAllText(testFile, "feature content");
            await TestUtilities.RunGitCommandAsync(tempPath, "add test.txt");
            await TestUtilities.RunGitCommandAsync(tempPath, "commit -m \"Feature commit\"");
            ...
            await TestUtilities.RunGitCommandAsync(tempPath, "add main.txt");
            await TestUtilities.RunGitCommandAsync(tempPath, "commit -m \"Main commit\"");
            await TestUtilities.RunGitCommandAsync(tempPath, "tag v1.5.0");  // Lower version
            
            // Merge feature branch
            await TestUtilities.RunGitCommandAsync(tempPath, "merge feature --no-ff --no-edit");
    ...
    public async Task LookupVersionLabel_MultipleMerges_ChoosesLargestVersion()
    {
        ...
        try
        {
            ...
            await TestUtilities.RunGitCommandAsync(tempPath, "checkout -b branchA");
            File.WriteAllText(testFile, "branch A content");
            await TestUtilities.RunGitCommandAsync(tempPath, "add test.txt");
            await TestUtilities.RunGitCommandAsync(tempPath, "commit -m \"Branch A commit\"");
            await TestUtilities.RunGitCommandAsync(tempPath, "tag v2.0.0");
            
            // Create branch B with version 3.0.0 (should be largest)
            await TestUtilities.RunGitCommandAsync(tempPath, "checkout main");
            await TestUtilities.RunGitCommandAsync(tempPath, "checkout -b branchB");
            var branchBFile = Path.Combine(tempPath, "branchB.txt");
            File.WriteAllText(branchBFile, "branch B content");
            await TestUtilities.RunGitCommandAsync(tempPath, "add branchB.txt");
            await TestUtilities.RunGitCommandAsync(tempPath, "commit -m \"Branch B commit\"");
            await TestUtilities.RunGitCommandAsync(tempPath, "tag v3.0.0");
            
            // Create branch C with version 1.5.0 (smaller)
            await TestUtilities.RunGitCommandAsync(tempPath, "checkout main");
            await TestUtilities.RunGitCommandAsync(tempPath, "checkout -b branchC");
            var branchCFile = Path.Combine(tempPath, "branchC.txt");
            File.WriteAllText(branchCFile, "branch C content");
            await TestUtilities.RunGitCommandAsync(tempPath, "add branchC.txt");
            await TestUtilities.RunGitCommandAsync(tempPath, "commit -m \"Branch C commit\"");
            await TestUtilities.RunGitCommandAsync(tempPath, "tag v1.5.0");
            
            // Merge all branches into main
            await TestUtilities.RunGitCommandAsync(tempPath, "checkout main");
            await TestUtilities.RunGitCommandAsync(tempPath, "merge branchA --no-ff --no-edit");
            await TestUtilities.RunGitCommandAsync(tempPath, "merge branchB --no-ff --no-edit");
            await TestUtilities.RunGitCommandAsync(tempPath, "merge branchC --no-ff --no-edit");
    ...
    public async Task LookupVersionLabel_DeepMergeHistory_PerformsCorrectly()
    {
        ...
        try
        {
            ...
            for (int i = 1; i <= 5; i++)
            {
                ...
                if (i == 3)
                    await TestUtilities.RunGitCommandAsync(tempPath, "tag v2.5.0");
                else if (i == 5)
                    await TestUtilities.RunGitCommandAsync(tempPath, "tag v3.1.0");  // Should be largest
    ...
    public async Task LookupVersionLabel_NoMergeCommitsWithTags_IncrementsFromBase()
    {
        ...
        try
        {
            ...
            for (int i = 1; i <= 3; i++)
            {
                ...
                await TestUtilities.RunGitCommandAsync(tempPath, "add test.txt");
                await TestUtilities.RunGitCommandAsync(tempPath, $"commit -m \"Feature commit {i}\"");
            }
    ...
    public async Task LookupVersionLabel_OctopusMerge_ChoosesLargestVersion()
    {
        ...
        try
        {
            ...
            {
                await TestUtilities.RunGitCommandAsync(tempPath, $"checkout -b {branchName}");
                var branchFile = Path.Combine(tempPath, $"{branchName}.txt");
                File.WriteAllText(branchFile, $"{branchName} content");
                await TestUtilities.RunGitCommandAsync(tempPath, $"add {branchName}.txt");
                await TestUtilities.RunGitCommandAsync(tempPath, $"commit -m \"{branchName} commit\"");
                await TestUtilities.RunGitCommandAsync(tempPath, $"tag {tagName}");
                await TestUtilities.RunGitCommandAsync(tempPath, "checkout main");
            }
            
            // Perform octopus merge (merge multiple branches at once)
            await TestUtilities.RunGitCommandAsync(tempPath, "merge branchA branchB branchC --no-ff --no-edit");
    ...
    public async Task LookupVersionLabel_NestedMerges_HandlesRecursion()
    {
        ...
        try
        {
            ...
            await TestUtilities.RunGitCommandAsync(tempPath, "checkout -b branch1");
            File.WriteAllText(testFile, "branch1");
            await TestUtilities.RunGitCommandAsync(tempPath, "add test.txt");
            await TestUtilities.RunGitCommandAsync(tempPath, "commit -m \"Branch1 commit\"");
            await TestUtilities.RunGitCommandAsync(tempPath, "tag v2.0.0");
            
            // Create branch2 from branch1 with higher tag
            await TestUtilities.RunGitCommandAsync(tempPath, "checkout -b branch2");
            var branch2File = Path.Combine(tempPath, "branch2.txt");
            File.WriteAllText(branch2File, "branch2");
            await TestUtilities.RunGitCommandAsync(tempPath, "add branch2.txt");
            await TestUtilities.RunGitCommandAsync(tempPath, "commit -m \"Branch2 commit\"");
            await TestUtilities.RunGitCommandAsync(tempPath, "tag v3.0.0");  // Highest
            
            // Merge branch2 back to branch1
            await TestUtilities.RunGitCommandAsync(tempPath, "checkout branch1");
            await TestUtilities.RunGitCommandAsync(tempPath, "merge branch2 --no-ff --no-edit");
            
            // Merge branch1 to main
            await TestUtilities.RunGitCommandAsync(tempPath, "checkout main");
            await TestUtilities.RunGitCommandAsync(tempPath, "merge branch1 --no-ff --no-edit");
