...
* 3.24.0:
  * GitReader を 1.18.0 に更新。
* 3.23.0:
  * GitReader を 1.17.0 に更新。
* 3.22.0:
  * パッケージビルド時に並列ビルドで失敗する場合があったのを修正 (#28)
  * 新しいプロパティ収集モードを実装。
    従来の方法は.NETのバージョンが上がる場合に失敗する可能性があるため、いずれ非推奨-->完全移行を行います。
    詳しくは `RelaxVersionerPropertyCollectionMode` を参照して下さい。
* 3.21.0:
  * 3.20.0でも.NET 9へのフォールバックで問題なく動作していましたが、正式に.NET 10をサポート。
* 3.20.0:
  * Annotated tagが含まれる場合を考慮したコードの改善。
* 3.19.0:
